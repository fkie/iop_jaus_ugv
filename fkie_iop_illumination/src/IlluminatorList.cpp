...
void IlluminatorList::init(std::shared_ptr<iop::Component> cmp)
{
    ...
    if (supported)
        p_illuminator_map["Headlights"]->init(cmp, "head_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["LeftTurnSignal"]->init(cmp, "left_turn_signal", state, "LeftTurnSignal");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["RightTurnSignal"]->init(cmp, "right_turn_signal", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["RunningLights"]->init(cmp, "running_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["BrakeLights"]->init(cmp, "brake_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["BackupLights"]->init(cmp, "backup_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["VisibleLightSource"]->init(cmp, "visible_light_source", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["IRLightSource"]->init(cmp, "ir_light_source", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["VariableLight1"]->init(cmp, "variable_light_1", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["VariableLight2"]->init(cmp, "variable_light_2", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["VariableLight3"]->init(cmp, "variable_light_3", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["VariableLight4"]->init(cmp, "variable_light_4", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["HighBeams"]->init(cmp, "high_beams", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["ParkingLights"]->init(cmp, "parking_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["FogLights"]->init(cmp, "fog_lights", state, "Headlights");
    parse_illuminator_def(value, supported, state);
    if (supported)
        p_illuminator_map["HazardLights"]->init(cmp, "hazard_lights", state, "Headlights");
